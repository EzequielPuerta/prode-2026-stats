<script lang="ts">
	...
	const order = $derived(
		sorted ? [...rows].sort((a, b) => b.diff - a.diff).map((r) => r.name) : orderedNames
	);
...
>
	...
	<BarY
		...
		x="name"
		y="diff"
		fill={(d: ComparisonRow) =>
			d.name === hoveredPlayer
				? 'white'
				: d.diff > 0
					? 'var(--color-success)'
					: d.diff < 0
...
{#if tooltip}
	...
	>
		...
		<div class="text-base-content/70">
			{baseLabel}: <span class="text-base-content font-medium">{tooltip.row.baseRank}º</span>
		</div>
		<div class="text-base-content/70">
			{targetLabel}: <span class="text-base-content font-medium">{tooltip.row.targetRank}º</span>
		</div>
		<div

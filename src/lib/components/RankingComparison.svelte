<script lang="ts">
	...
	import type { ParsedProde, ParsedDoubles, Scenario, Standing } from '$lib/types';
	...
	const rows = $derived.by(() => {
		...
		return diffs.map((r): Row => ({ ...r, points: pointsByName.get(r.name) ?? 0 }));
	});

	const orderedNames = $derived(rows.map((r) => r.name));
	const yDomain = [20, -25];

	const histMaxPoints = $derived(rows.length ? Math.max(...rows.map((r) => r.points)) : 0);
	const histYDomain = $derived([0, histMaxPoints * 1.1 || 1]);
	...
	const histChartHeight = $derived(Math.max(120, histWrapHeight));
	...
	let histScales = $state<Scales | null>(null);
	...
	const histTooltip = $derived.by(() => {
		if (!hoveredPlayer || !histScales) return null;
		const row = rows.find((r) => r.name === hoveredPlayer);
		if (!row) return null;
		const bandwidth = histScales.x.fn.bandwidth?.() ?? 0;
		const x = histScales.x.fn(row.name) + bandwidth / 2;
		const y = histScales.y.fn(row.points);
		return { row, x, y };
	});
</script>
...
<div class="flex h-full flex-col gap-4">
	{#if stats}
		<div class="stats stats-horizontal w-full shrink-0 overflow-x-auto shadow">
			<div class="stat w-32 px-4 py-3 lg:w-50 lg:px-6 lg:py-4">
				<div class="stat-title truncate text-xs lg:text-sm">Diferencia promedio abs.</div>
			...
			<div class="stat w-28 px-4 py-3 lg:w-40 lg:px-6 lg:py-4">
				...
				<div class="stat-desc truncate">cambian de puesto</div>
			</div>
		</div>
	{/if}

	<div class="relative min-h-0 flex-2" bind:clientHeight={histWrapHeight}>
		<Plot
			height={histChartHeight}
			marginBottom={8}
			marginLeft={40}
			x={{ domain: orderedNames, axis: false }}
			y={{ label: 'Puntos', grid: true, domain: [0, 45] }}
		>
			<PlotProbe onscales={(s) => (histScales = s as Scales)} />
			<BarY data={rows} x="name" y="points" fill="var(--color-primary)" />
		...
		{#if histTooltip}
			<div
				...
				style="left: {histTooltip.x}px; top: {histTooltip.y}px; transform: translate(-50%, calc(-100% - 14px));"
			>
				<div class="mb-1 font-semibold">{histTooltip.row.name}</div>
				<div class="text-base-content/70">
					{target.label}:
					<span class="text-base-content font-medium">{histTooltip.row.points} pts</span>
				</div>
			</div>
		{/if}
	</div>

	<div class="relative min-h-0 flex-3" bind:clientHeight={wrapHeight}>
		<Plot
			height={chartHeight}
			marginBottom={92}
			marginLeft={40}
